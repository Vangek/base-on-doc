<template>
  <div class="component-doc">
    <article class="component-doc__article">
      ...
      <p>
        以下为按需引入 <code>base-on-ui</code> 的 <strong>BaseFormJson</strong>：左侧为物料区（可拖拽），中间为表单画布；为便于阅读，右侧属性面板已关闭（<code>hide-config</code>）。下方可查看当前表单数据。
      </p>

      <div class="component-doc__demo component-doc__demo--form-json">
        <UiBaseFormJson
          ref="formRef"
          v-model="formData"
          :config="formConfig"
        ...
        <p class="component-doc__hint">
          当前 <code>modelValue</code>：<code>{{ formJson }}</code>
        </p>
      </div>
...
<script setup name="BaseFormJsonDoc">
import { computed, onMounted, reactive, ref } from 'vue'
import { useRoute } from 'vue-router'
import { ElMessage } from 'element-plus'
import { Reading } from '@element-plus/icons-vue'
import { BaseFormJson as UiBaseFormJson } from 'base-on-ui'
import 'base-on-ui/style.css'
import { renderDocMarkdown } from '@/utils/markdown'
import docSource from '@/ReadMe/base-form-json.md?raw'
import demoSource from '@/views/components/snippets/base-form-json-demo-source.txt?raw'
  ...
  formItems: [
    {
      ...
      span: 24,
      value: '示例',
      options: { placeholder: '可从左侧拖拽更多控件到画布' },
    },
  ],
})

const formJson = computed(() => JSON.stringify(formData))
