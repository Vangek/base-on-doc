<template>
  <div class="component-doc">
    <article class="component-doc__article">
      ...
      <p>
        以下为按需引入 <code>base-on-ui</code> 的 <strong>BasePagination</strong>：修改页码或每页条数时，会通过 <code>handlePagination</code> 回传新的
        <code>pageNum</code> / <code>pageSize</code>。
      </p>

      <div class="component-doc__demo component-doc__demo--pagination">
        <UiBasePagination
          :total="total"
        ...
        <p class="component-doc__hint">
          当前 <code>pagination</code>：<code>{{ paginationText }}</code>
        </p>
        <p class="component-doc__hint">
          最近一次 <code>handlePagination</code> 参数：<code>{{ lastEventText }}</code>
        </p>
      </div>
...
<script setup name="BasePaginationDoc">
import { computed, onMounted, reactive, ref } from 'vue'
import { useRoute } from 'vue-router'
import { Reading } from '@element-plus/icons-vue'
import { BasePagination as UiBasePagination } from 'base-on-ui'
import 'base-on-ui/style.css'
import { renderDocMarkdown } from '@/utils/markdown'
import docSource from '@/ReadMe/base-pagination.md?raw'
import demoSource from '@/views/components/snippets/base-pagination-demo-source.txt?raw'
...
const lastEventText = ref('（尚未触发）')

const paginationText = computed(() => JSON.stringify(pagination))

function onPagination(p) {
  Object.assign(pagination, p)
  lastEventText.value = JSON.stringify(p)
}
...
.component-doc__demo--pagination {
  padding: 16px;
}
